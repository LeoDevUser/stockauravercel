...
export function TradingVerdict({
  results,
  transactionCost,
  tradeSize,
  riskTolerance,
  ...
  const isShortSignal = results.final_signal
    ? results.final_signal.includes('SHORT')
    : false
  ...
  const effectiveShares = isSpeculativeSignal
    ? Math.floor(suggestedShares / 2)
    : suggestedShares
  ...
  > = {
    ...
    SHORT_DOWNTREND: {
      ...
      confidence: 'HIGH',
      summary:
        'Strong downtrend with persistent momentum and ideal entry level',
    },
    SHORT_BOUNCES_ONLY: {
      verdict: 'TRADEABLE',
    ...
    WAIT_PULLBACK: {
      ...
      confidence: 'MEDIUM',
      summary:
        'Stock overbought (Z-EMA > 1.0) - wait for pullback before buying',
    },
    WAIT_SHORT_BOUNCE: {
      verdict: 'WAIT',
    ...
    WAIT_OR_SHORT_BOUNCE: {
      ...
      confidence: 'MEDIUM',
      summary:
        'Overbought in mean-reverting uptrend — wait for a pullback to buy the dip, or short the bounce now',
    },
    WAIT_FOR_REVERSAL: {
      verdict: 'WAIT',
    ...
    WAIT_FOR_TREND: {
      ...
      confidence: 'LOW',
      summary:
        'Strong momentum but no clear trend yet - wait for trend confirmation',
    },
    NO_CLEAR_SIGNAL: {
    ...
    SPEC_BUY_UPTREND: {
      ...
      confidence: 'LOW',
      summary:
        'Uptrend with momentum detected, but only 2/5 statistical tests passed — use half position size',
    },
    SPEC_BUY_PULLBACK: {
      verdict: 'SPECULATIVE',
      ...
      confidence: 'LOW',
      summary:
        'Pullback entry in uptrend, but only 2/5 statistical tests passed — use half position size',
    },
    SPEC_BUY_MOMENTUM: {
      verdict: 'SPECULATIVE',
      ...
      confidence: 'LOW',
      summary:
        'Momentum detected in uptrend, but only 2/5 statistical tests passed — use half position size',
    },
    SPEC_SHORT_DOWNTREND: {
      ...
      confidence: 'LOW',
      summary:
        'Downtrend continuing, but only 2/5 statistical tests passed — use half position size',
    },
    SPEC_SHORT_BOUNCES_ONLY: {
      ...
      confidence: 'LOW',
      summary:
        'Downtrend bounce short, but only 2/5 statistical tests passed — use half position size',
    },
    SPEC_SHORT_MOMENTUM: {
      verdict: 'SPECULATIVE',
      ...
      confidence: 'LOW',
      summary:
        'Downtrend momentum, but only 2/5 statistical tests passed — use half position size',
    },
    SPEC_WAIT_OR_SHORT_BOUNCE: {
      ...
      confidence: 'LOW',
      summary:
        'Uptrend weakening, but only 2/5 statistical tests passed — use half position size if trading',
    },
    SPEC_WAIT_FOR_REVERSAL: {
      ...
      confidence: 'LOW',
      summary:
        'Downtrend weakening, but only 2/5 statistical tests passed — use half position size if trading',
    },
  }
  ...
  if (results.vp_confirming === false) {
    failureReasons.push({
      metric: 'Volume-Price',
      value:
        results.vp_ratio !== null
          ? `Ratio: ${results.vp_ratio.toFixed(2)}`
          : 'N/A',
  ...
  if (isSpeculative) {
    if (results.predictability_score < 3) {
      ...
      if (results.mean_rev_up !== null && results.mean_rev_down !== null) {
        if (
          Math.abs(results.mean_rev_up) > 0.003 &&
          Math.abs(results.mean_rev_down) > 0.003
        )
          passed.push('Mean Reversion')
      ...
      if (failed.length > 0)
        specWarnings.push(`Failed tests: ${failed.join(', ')}`)
      if (passed.length > 0)
        specWarnings.push(`Passed tests: ${passed.join(', ')}`)
    }
  }
  ...
  return (
    <div className="trading-verdict-container">
      ...
      >
        <div className="verdict-header">
          ...
          <div className="verdict-badges">
            <span
              className={`badge risk-${config.riskLevel.toLowerCase().replace('-', '_')}`}
            >
            ...
            <div className="metrics-grid-compact">
              <div className="metric-item">
                ...
                <strong style={{ color: '#22c55e' }}>
                  {results.predictability_score}/5
                </strong>
              </div>
              ...
              {results.momentum_corr !== null && (
                <div className="metric-item">
                  <span>Momentum</span>
                  <strong style={{ color: '#22c55e' }}>
                    {(results.momentum_corr * 100).toFixed(1)}%
                  </strong>
                </div>
              )}
              {results.vp_confirming !== null && (
                <div className="metric-item">
                  ...
                  <strong
                    style={{
                      color: results.vp_confirming ? '#22c55e' : '#f59e0b',
                    }}
                  >
                    {results.vp_confirming ? '✓' : '✗'}{' '}
                    {results.vp_ratio?.toFixed(2)}
                  </strong>
                </div>
              )}
            </div>
            ...
            >
              ...
              <span
                style={{
                  ...
                  fontWeight: 'bold',
                  color:
                    results.trade_quality >= 7
                      ? '#22c55e'
                      : results.trade_quality >= 5
                        ? '#f59e0b'
                        : results.trade_quality >= 3
                          ? '#f97316'
                          : '#ef4444',
                }}
              >
              ...
              <span
                style={{
                  ...
                  borderRadius: '4px',
                  background:
                    results.quality_label === 'Excellent'
                      ? 'rgba(34,197,94,0.15)'
                      : results.quality_label === 'Good'
                        ? 'rgba(245,158,11,0.15)'
                        : results.quality_label === 'Fair'
              ...
              >
                <span>
                  ...
                  <strong style={{ color: '#ccc' }}>
                    {results.quality_components.trend_alignment.toFixed(1)}/2
                  </strong>
                  <Tooltip
                    content={
                      <>
                        <strong>Trend Alignment (0–2)</strong>
                        <p>
                          Measures how well momentum direction matches the
                          current trend.
                        </p>
                        <p>
                          <strong>2.0:</strong> Strong agreement |{' '}
                          <strong>1.5:</strong> Moderate | <strong>1.0:</strong>{' '}
                          Weak | <strong>0.0:</strong> Conflicting
                        </p>
                      </>
                    }
                  />
                ...
                <span>
                  Entry Timing:{' '}
                  <strong style={{ color: '#ccc' }}>
                    {results.quality_components.entry_timing.toFixed(1)}/2
                  </strong>
                  <Tooltip
                    content={
                      <>
                        <strong>Entry Timing / Z-EMA (0–2)</strong>
                        <p>
                          Evaluates entry timing using Z-score relative to
                          exponential moving average.
                        </p>
                        <p>
                          <strong>2.0:</strong> Sweet spot |{' '}
                          <strong>1.5:</strong> Moderate | <strong>0.5:</strong>{' '}
                          Overbought/oversold
                        </p>
                      </>
                    }
                ...
                <span>
                  Sharpe:{' '}
                  <strong style={{ color: '#ccc' }}>
                    {results.quality_components.sharpe_quality.toFixed(1)}/2
                  </strong>
                  <Tooltip
                    content={
                      <>
                        <strong>Sharpe Quality (0–2)</strong>
                        <p>Risk-adjusted return quality.</p>
                        <p>
                          <strong>2.0:</strong> Sharpe ≥ 1.5 |{' '}
                          <strong>1.0:</strong> Sharpe ≥ 0.5 |{' '}
                          <strong>0.5:</strong> Sharpe &gt; 0
                        </p>
                      </>
                    }
                  />
                ...
                <span>
                  Vol Fit:{' '}
                  <strong style={{ color: '#ccc' }}>
                    {results.quality_components.volatility_fit.toFixed(1)}/2
                  </strong>
                  <Tooltip
                    content={
                      <>
                        <strong>Volatility Fit (0–2)</strong>
                        <p>Whether volatility is in the ideal trading range.</p>
                        <p>
                          <strong>2.0:</strong> 20–35% | <strong>1.5:</strong>{' '}
                          15–45% | <strong>0.3:</strong> Outside range
                        </p>
                      </>
                    }
                  />
                ...
                <span>
                  Vol-Price:{' '}
                  <strong style={{ color: '#ccc' }}>
                    {results.quality_components.volume_confirmation.toFixed(1)}
                    /2
                  </strong>
                  <Tooltip
                    content={
                      <>
                        <strong>Volume-Price Confirmation (0–2)</strong>
                        <p>Whether volume supports the trend direction.</p>
                        <p>
                          <strong>2.0:</strong> Strong confirmation |{' '}
                          <strong>0.2:</strong> No confirmation
                        </p>
                      </>
                    }
                  />
          ...
          <div className="speculative-details" style={{ marginTop: '1em' }}>
            ...
            >
              <strong style={{ color: '#f97316' }}>
                ⚠ Speculative Signal — Reduced Confidence
              </strong>
              <p
                style={{
                  color: '#d0d0d0',
              ...
              >
                Only {results.predictability_score}/5 statistical tests passed
                (high-conviction requires 3/5).
              ...
              {results.regime_stability !== null && (
                <div className="metric-item">
                  ...
                  <strong
                    style={{
                      color:
                        results.regime_stability >= 1.0 ? '#22c55e' : '#f59e0b',
                    }}
                  >
                    {(results.regime_stability * 100).toFixed(0)}%
                  </strong>
        ...
        <div className="position-details">
          ...
          {canExecute ? (
            <>
              <div className="position-grid">
                <div className="position-item">
                  ...
                  <span className="value">
                    {effectiveShares} shares @ ${results.current?.toFixed(2)}
                  </span>
                  <small>
                    Position: $
                    {(effectiveShares * (results.current || 0)).toFixed(2)}
                  </small>
                </div>
                <div className="position-item">
                  ...
                  {noStopLoss ? (
                    ...
                    <>
                      ...
                      <small>
                        ...
                        {isShortSignal ? 'above' : 'below'} entry
                      </small>
                    </>
                  )}
                </div>
                <div className="position-item">
                  ...
                  <span className="value">
                    $
                    {(effectiveShares * results.current * riskPerTrade).toFixed(
                      2
                    )}
                  </span>
                  <small>
                    {actualRiskPct.toFixed(1)}% of ${tradeSize.toLocaleString()}{' '}
                    trade
              ...
              {positionSizeNote && (
                <div className="position-note">
                  <p
                    style={{
                      color: isSpeculative ? '#f97316' : '#f59e0b',
                      fontSize: '0.9em',
                      marginTop: '1em',
                      lineHeight: '1.5',
                    }}
            ...
            <div className="position-warning">
              <p
                style={{ color: '#ef4444', margin: '1em 0', fontSize: '1.1em' }}
              >
              ...
              >
                The trading pattern is still valid. Increase your trade size to
                execute.
              </p>
            </div>
          )}
        ...
        <div className="context-grid">
          ...
          <div className="context-item">
            ...
            <span className="context-value">
              ...
              {results.recent_return_1y !== null &&
                ` (${(results.recent_return_1y * 100).toFixed(1)}% 1Y)`}
            </span>
          </div>
        </div>
      </div>
      ...
      <div className="liquidity-costs">
        ...
        <div className="costs-grid">
          ...
          <div className="cost-item">
            ...
            <span
              className={
                positionSizeVsVolume && positionSizeVsVolume > 0.02
                  ? 'warning'
                  : ''
              }
            >
      ...
      <div className="verdict-disclaimer">
        <p>
          <strong>⚠ Important:</strong> Historical patterns don't guarantee
          future performance.
          {config.verdict === 'TRADEABLE' &&
            ' Paper trade first to validate the strategy in real conditions.'}
          {config.verdict === 'WAIT' &&
            ' Wait for the specified setup before entering.'}
          {config.verdict === 'DO NOT TRADE' &&
            ' This stock does not meet minimum statistical requirements for reliable trading.'}
        </p>
      </div>
    </div>
  )
